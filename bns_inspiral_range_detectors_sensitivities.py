 ...
 #   freq_L1, psd_L1     = np.loadtxt(f'{path}/aligo_O4high.txt', unpack=True)
  ...
  #  outdata = np.array([freq_L1, asd_L1]).T
   ...
   # np.savetxt(X = outdata, fname = os.path.join(f'{path}', 'asd_L1.txt'))
    ...
    pyplot.loglog(freq_L1, asd_L1, label=r'$\mathrm{LIGO}\,\mathrm{L1}$', color=colors['L1'], linewidth=1, alpha=0.7)
    pyplot.loglog(freq_H1, asd_H1, label=r'$\mathrm{LIGO}\,\mathrm{H1}$', color=colors['H1'], linewidth=1, alpha=0.7)
    ...
    range_L1 = inspiral_range(FrequencySeries(asd_L1**2, f0=freq_L1[0], df=freq_L1[1]-freq_L1[0]), snr =8, fmin=10, mass1=1.4, mass2=1.4).value
    range_V1 = inspiral_range(FrequencySeries(asd_V1**2, f0=freq_V1[0], df=freq_V1[1]-freq_V1[0]), snr =8, fmin=10, mass1=1.4, mass2=1.4).value
    ...
    range_H1 = inspiral_range(FrequencySeries(asd_H1**2, f0=freq_H1[0], df=freq_L1[1]-freq_L1[0]), snr =8, fmin=10, mass1=1.4, mass2=1.4).value
