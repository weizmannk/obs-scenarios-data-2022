...
with tqdm(total=len(run_names)) as progress:
    for run_name in run_names:
        ...
        # farah data
        bns_farah   = Table(tables[run_name]['bns_farah'])
        ...
        axs[0].text(0.05, 0.9, f"{pop.upper().split('_')[0]}", transform=axs[0].transAxes, color ='g', va='top', fontname="Times New Roman", size=13, fontweight="bold")
        ...
        axs[1].text(0.05, 0.9, f"{pop.upper().split('_')[0]}", transform=axs[1].transAxes, color ='g', va='top', fontname="Times New Roman", size=13, fontweight="bold")
