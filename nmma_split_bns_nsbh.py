    ...
    for dist in distribution:
        ...
        if dist == 'Farah':
            ...
            for run_name, run_dir in zip(run_names, run_dirs):
                ...
                path = Path(f'/home/kiendrebeogo/weizmann-doc/GitHub/runs_HL_O4a_SNR_8_PSD_Ideal/runs/{run_dir}/farah')
                print(path)
                ...
                for pop in pops:
                    ...
                    xml_data = gwpy_Table.read(str(path/'events.xml.gz'), format="ligolw", tablename="sim_inspiral")
                    ...
                    time_dict = {'simulation_id':simulation_ID, 'gps_time':gps_time, 'geocent_time':geocent_end_time, 'geocent_time_ns':geocent_end_time_ns}
                    time_table = Table(time_dict)
                    
                    tables = join(data, time_table)

                    tables.write(Path(f"{pop_dir}/{pop.lower()}_{run_name}_injections.dat"), format='ascii.tab', overwrite=True)
                ...
                del injections, table, source_mass1, source_mass2, z, zp1,  data, time_dict, time_table, tables, path
