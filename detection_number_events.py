...
for dist in distributions:
    if dist == 'Farah':
        for run_name, run_dir in zip(tqdm(run_names), run_dirs):
            ...
            print(f'the Farah (GWTC-3)  run {run_name}')
            
            Frarah_table[run_name] = {}
            ...
            for pop in pops:
                ...
                Frarah_table[run_name][pop] = len(data)
        ...
        for run_name, run_dir in zip(tqdm(run_names), run_dirs):
            ...
            Petrov_table[run_name] = {}
        
            for pop in pops:
                      
                print(f'the  Petrov (LRR) {pop} in the  run {run_name}')
                ...
                Petrov_table[run_name][pop] = len(table)
...
for run_name in run_names:
    ...
    BNS_detect.append(Frarah_table[run_name]['BNS'])
    NSBH_detect.append(Frarah_table[run_name]['NSBH'])
    BBH_detect.append(Frarah_table[run_name]['BBH'])
    ...
    bns_astro.append(Petrov_table[run_name]['BNS'])
    nsbh_astro.append(Petrov_table[run_name]['BNS'])
    bbh_astro.append(Petrov_table[run_name]['BNS'])
